...
estRMT <- function(
  ...
  cutoff <- cutoff[1]
  if (!cutoff %in% c("max", "each")) stop("Invalid cutoff")
  ...
  sum <- 0
  for (i in 1:ncol(vec)) sum <- sum + val[i] * vec[, i] %*% t(vec[, i])
...
getDenoisedCorMatrix <- function(
  ...
  if (is.null(prior.means)) {
    ...
    message(
      "Assuming the prior means passed were derived from the full sample set."
    )
  }
  ...
  #tidy up
  colnames(cor.mat.denoise) <- rownames(
    cor.mat.denoise
  ) <- as.character(granges(cor.mat$gr.cor))
...
plotCorMatrix <- function(
  ...
  p <- ggplot(
    cor.mat.melt,
    aes(x = .data$Var2, y = .data$Var1, fill = .data$value)
  ) +
    geom_raster() +
